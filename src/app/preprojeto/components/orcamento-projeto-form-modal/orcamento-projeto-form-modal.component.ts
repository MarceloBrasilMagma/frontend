...
import {
  ...
  PreProjetoOrcamentoAlterarCommand,
  PreProjetoOrcamentoCriarCommand,
  PreProjetosClient,
  ...
  PreProjetoOrcamentoAlterarSituacaoCommand,
} from 'web-api-client';

@Component({
  selector: 'app-orcamento-projeto-form-modal',
  templateUrl: './orcamento-projeto-form-modal.component.html',
  styleUrls: ['./orcamento-projeto-form-modal.component.scss'],
})
export class OrcamentoProjetoFormModalComponent implements OnInit {
  @Input() projetoId: number;
  @Input() orcamento: PreProjetoOrcamentoVm;
  @Input() declaracaoSituacao: DeclaracaoTrabalhoSituacao;

  SituacaoPreProjetoOrcamento = SituacaoPreProjetoOrcamento;
  ...
  private cadastrar(situacao?: SituacaoPreProjetoOrcamento) {
    this.saving = true;
    this.PreProjetosClient
      .incluirOrcamento(
        PreProjetoOrcamentoCriarCommand.fromJS({
          ...this.form.value,
  ...
  private atualizar(situacao?: SituacaoPreProjetoOrcamento) {
    this.saving = true;

    if (!this.orcamento.declaracaoTrabalhoId) {
      this.PreProjetosClient
        .alterarOcamento(
          PreProjetoOrcamentoAlterarCommand.fromJS({
            ...this.form.value,
            projetoId: this.projetoId,
          })
        )
    ...
    this.formSubscription = this.form.valueChanges
      ...
      .subscribe((val) => {
        ...
        if (
          ...
          SituacaoPreProjetoOrcamento.AguardandoClassificacao
        ) {
  ...
  get exibirObservacoes(): boolean {
    return (
      ...
      SituacaoPreProjetoOrcamento.AguardandoClassificacao ||
      this.orcamento?.situacao ===
      SituacaoPreProjetoOrcamento.ClassificacaoRealizada
    );
  }
  ...
  get exibirEnviarClassificacao(): boolean {
    return (
      !this.orcamento ||
      this.orcamento.situacao === SituacaoPreProjetoOrcamento.EmElaboracao
    );
  }
